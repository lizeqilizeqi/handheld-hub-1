...
function hh_admin_t($key)
{
    ...
    if ($map === null) {
        $map = array(
            ...
            'deploy' => '服务器部署',
            'api_settings' => 'API 管理',
            'api_settings_hint' => '管理外部游戏元数据 API（IGDB 等）。密钥保存在 config.secrets.php，勿提交 Git。',
            'api_settings_game_use_igdb' => '复古游戏（Game）模块使用 IGDB 作为元数据源',
            'api_settings_csrf_error' => '表单已过期，请刷新后重试。',
            'sites' => '站点 / Hub',
