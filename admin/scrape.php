...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $channel = isset($_POST['channel']) ? (string) $_POST['channel'] : 'handheld';
    $options = array();
    if ($channel === 'news') {
        $since = isset($_POST['news_since']) ? trim((string) $_POST['news_since']) : date('Y') . '-01-01';
    ...
    try {
        $jobId = hh_scraper_queue_job($mode, $slug !== '' ? $slug : null, $channel, $options);
...
<div class="card scrape-running" id="scrape-monitor" data-job-id="<?php echo (int) $watchJob['id']; ?>">
  ...
  <ul class="scrape-progress" id="scrape-progress">
    ...
    <li>已发现：<span id="scrape-found"><?php echo (int) $watchJob['items_found']; ?></span> <?php echo hh_h(hh_scrape_channel_unit($watchJob)); ?></li>
...
<div class="card">
  ...
  <form method="post" class="grid-2">
    <div>
      <label><?php echo hh_h(hh_admin_t('scrape_channel')); ?></label>
      <select name="channel"<?php echo $runningJob ? ' disabled' : ''; ?>>
        <option value="handheld"><?php echo hh_h(hh_admin_t('scrape_channel_handheld')); ?></option>
        <option value="news"><?php echo hh_h(hh_admin_t('scrape_channel_news')); ?></option>
        <option value="game"><?php echo hh_h(hh_admin_t('scrape_channel_game')); ?></option>
      </select>
      ...
      <div id="news-scrape-since" class="news-scrape-since">
        <label><?php echo hh_h(hh_admin_t('scrape_news_since')); ?></label>
        <input type="date" name="news_since" value="<?php echo hh_h($newsSinceDefault); ?>"<?php echo $runningJob ? ' disabled' : ''; ?>>
        <p class="muted"><?php echo hh_h(hh_admin_t('scrape_news_since_hint')); ?></p>
      </div>
    ...
    <div>
      ...
      <p class="muted">资讯：快科技掌机游戏列表（HTML）。游戏：源未配置时会提示失败。</p>
    </div>
  </form>
</div>
...
<div class="card">
  ...
  <table>
    <tr><th>ID</th><th><?php echo hh_h(hh_admin_t('scrape_channel')); ?></th><th><?php echo hh_h(hh_admin_t('job_type')); ?></th><th><?php echo hh_h(hh_admin_t('status')); ?></th><th><?php echo hh_h(hh_admin_t('new')); ?></th><th><?php echo hh_h(hh_admin_t('updated')); ?></th><th><?php echo hh_h(hh_admin_t('failed')); ?></th><th><?php echo hh_h(hh_admin_t('job_result')); ?></th><th></th></tr>
    <?php foreach ($jobs as $j): ?>
    <tr>
      <td><?php echo (int) $j['id']; ?></td>
      <td><?php echo hh_h(isset($j['channel']) ? $j['channel'] : 'handheld'); ?></td>
...
(function () {
  ...
  var box = document.getElementById('news-scrape-since');
  if (!sel || !box) return;
  function sync() {
    box.style.display = sel.value === 'news' ? 'block' : 'none';
  }
